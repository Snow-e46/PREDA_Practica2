...
public class Lector {
	...
	// Leer parámetros desde un archivo
	public static int[] leerParametros() throws IOException, FileNotFoundException {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fichero))) {
			...
			if (linea == null || linea.isEmpty()) {
				throw new IOException("El fichero de entrada está vacío o tiene formato incorrecto.");
			}

			String[] partes = linea.split(" ");
			if (partes.length != 3) {
				throw new IOException("El fichero debe contener exactamente tres números enteros.");
			}
			...
			if (posteOrigen < 1 || posteOrigen > 3 || posteDestino < 1 || posteDestino > 3
					|| posteOrigen == posteDestino) {
				throw new IOException("Los postes deben estar entre 1 y 3, y ser diferentes entre sí.");
			}

			if (numDiscos <= 0) {
				throw new IOException("El número de discos debe ser un entero positivo.");
			}
